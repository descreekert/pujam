...
(function(){
    ...
    function initTabs() {
        ...
        if (audio && audio.tagName && audio.tagName.toLowerCase() == 'audio') {
            setTabFocus('remote');
        } else {
            setTabFocus('upload');
        }
    ...
    function initAudio(){
        ...
        (function(){
            ...
            if(img && img.className){
                var hasFakedClass = (img.className.indexOf("edui-faked-audio")!=-1),
     ...
     */
    function initOnline(){
        onlineFile = onlineFile || new OnlineFile('audioList');
        onlineFile.reset();
    ...
    OnlineFile.prototype = {
        ...
        pushData: function (list) {
            var i, item, audio, icon, _this = this,
            ...
            for (i = 0; i < list.length; i++) {
                if(list[i] && list[i].url) {
                    ...
                    audio.width = 100;
                    audio.height = 100;
                    audio.setAttribute('src', urlPrefix + list[i].url + (list[i].url.indexOf('?') == -1 ? '?noCache=':'&noCache=') + (+new Date()).toString(36) );
                    audio.setAttribute('_src', urlPrefix + list[i].url);
        ...
        getInsertList: function () {
            ...
            for (i = 0; i < lis.length; i++) {
                if (domUtils.hasClass(lis[i], 'selected')) {
                    var audio = lis[i].firstChild,
                        src = audio.getAttribute('_src');
                    list.push({
                        src: src,
                        _src: src,
    ...
    function insertSearch(id){
        ...
        for(var i=0,img; img=imgs[i++];){
            if(img.getAttribute("selected")){
                audioObjs.push({
                    src:img.getAttribute("ue_audio_url"),
                    width:420,
                    height:280,
                    align:"none"
                });
    ...
    function convert_url(url){
        ...
        url = utils.trim(url)
            ...
            .replace(/v\.qq\.com\/cover\/[\w]+\/[\w]+\/([\w]+)\.html/i, "static.audio.qq.com/TPout.swf?vid=$1")
            .replace(/v\.qq\.com\/.+[\?\&]vid=([^&]+).*$/i, "static.audio.qq.com/TPout.swf?vid=$1")
    ...
    function createPreviewAudio(url){
        ...
        if(conUrl)
        {
            $G("preview").innerHTML = '<audio class="previewAudio" ' +
                ' src="' + conUrl + '"' +
                ' width="' + 420  + '"' +
                ' height="' + 280  + '"' +
